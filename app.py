...
def safe_int(value, default=0) -> int:
    """将任意值（包括 NaN/inf/字符串/小数）安全转换为整数"""
    try:
        if value is None:
            return default
...
class ESGCarbonScoringSystem:
    ...
    def _generate_system_prompt(self) -> str:
        ...
        prompt += (
            ...
            '2. core_advantages 填写规则（参考国内排放标准合规示例）：\n'
            '   - 必须列出**所有得分为2分（满分）**的项目，**一个都不能少**。\n'
            '   - 每个条目采用以下固定结构（禁止使用【】方括号符号）：\n'
            '     "项目名称方面，[具体披露内容]。报告明确指出[具体标准编号/认证信息]，提供了[具体的定量数据/认证细节]，说明[合规实现方式]，这对[利益相关方]的[具体价值]具有重要价值。"\n'
            '   - 国内排放标准合规示例：国内排放标准合规方面，报告明确指出产品碳足迹证书符合GB/T 24067-2024《温室气体产品碳足迹量化要求和指南》以及T/CBMF 277-2024，提供了具体的标准编号和认证信息，说明公司产品在国内排放要求上已实现可核查的定量合规，这对客户和监管机构的信任具有重要价值。\n'
            '   - 如果**没有任何项目得分为2分**，则必须输出 **["无"]**。\n'
            '   - 绝对禁止输出空列表[]或只写"无"字而不带方括号。\n'
            '3. core_issues 填写规则（参考国际排放标准合规示例）：\n'
            '   - 必须列出**所有得分为0分**的项目，再列出**得分为1分**的项目（优先级：0分在前）。\n'
            '   - 每条采用以下结构（禁止使用【】方括号符号）：\n'
            '     "项目名称方面，[当前披露状态/缺失内容]。这可能导致[具体风险/后果]，增加[利益相关方]的[具体问题]。"\n'
            '   - 国际排放标准合规示例：国际排放标准合规披露不足，仅提及ISO 50001能源管理体系认证，缺乏对国际排放标准（如ISO 14064、EN标准）的定量合规数据，可能导致国际投资者和跨境合作伙伴对公司碳合规水平的认知不完整，增加信息不对称风险。\n'
            '   - 如果**所有项目都得2分**，则必须输出 **["无"]**。\n'
            '4. improvement_suggestions：必须针对core_issues中的每一个问题，一一对应提出建议，每条采用以下结构（禁止使用【】方括号符号）：\n'
...
# --- 页面 2: 企业深度画像 (修复 multiselect 失效) ---
elif page == "🏢 企业深度画像":
    st.title("企业深度画像")
    st.markdown("查询单企业历年ESG碳披露表现，进行多维度趋势分析与详细评分解读")
    
    # ===== 关键修复：使用 session_state 保存查询的公司代码 =====
    ...
    # 当按钮被点击时，把查询的代码保存到 session_state
    if search_button and input_code:
        st.session_state.queried_code = str(input_code).strip().zfill(6)
    
    # 使用 session_state 中保存的代码（持久化查询结果）
    if st.session_state.queried_code:
        queried_code = st.session_state.queried_code
        company_data = st.session_state.df[st.session_state.df['code'] == queried_code].sort_values('year')
        ...
        if company_data.empty:
            ...
            # ===== multiselect 现在可以正常工作了！=====
            selected_dimensions = st.multiselect(
                "选择要对比的维度（可多选）",
                options=all_dimensions,
                default=['最终得分'],
                key='dimension_multiselect'  # 使用 key 让控件状态持久化
            )
            ...
            with col1:
                ...
                <div class="metric-card">
                    ...
                    <p style="font-size:2rem; font-weight:700; margin:0; color:#10B981;">{final_score_int}</p>
                </div>
                """, unsafe_allow_html=True)
...
# --- 页面 3: 行业对标分析 (同样使用 session_state) ---
elif page == "📊 行业对标分析":
    st.title("行业对标分析")
    st.markdown("基于经济绩效与碳披露绩效的四象限分析，直观展示企业在行业中的定位")
    
    # ===== 同样修复：保存对标查询状态 =====
    if 'benchmark_code' not in st.session_state:
        st.session_state.benchmark_code = None
    if 'benchmark_year' not in st.session_state:
        st.session_state.benchmark_year = None
    if 'benchmark_econ' not in st.session_state:
        st.session_state.benchmark_econ = None
    ...
    if st.session_state.benchmark_code:
        ...
        target_df = st.session_state.df[(st.session_state.df['code'] == bm_code) & (st.session_state.df['year'] == bm_year)]
        ...
        if target_df.empty:
            ...
            if bm_econ_code not in target_df.columns:
                ...
                peer_df = st.session_state.df[
                    (st.session_state.df['industrycodec'] == industry) & 
                    (st.session_state.df['year'] == bm_year)
                ].dropna(subset=[bm_econ_code, '最终得分'])
            ...
            if len(peer_df) < 2:
                ...
                x_range = [peer_df[bm_econ_code].min(), peer_df[bm_econ_code].max()]
