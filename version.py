...
class ESGCarbonScoringSystem:
    ...
    def _generate_system_prompt(self) -> str:
        ...
        prompt += (
            ...
            '  "score_level": "优秀",\n'
            '  "summary": {\n'
            '    "comprehensive_evaluation": "综合评价正文（不少于250字）...",\n'
            '    "core_advantages": ["优势条目1（按固定结构）", "优势条目2"] 或 ["无"],\n'
            '    "core_issues": ["问题条目1（按固定结构，0分优先）", "问题条目2"] 或 ["无"],\n'
            '    "improvement_suggestions": ["建议条目1（与core_issues一一对应）", "建议条目2"] 或 ["无"]\n'
            '  }\n'
            '}\n'
            '```\n\n'
            '【输出前自查清单】\n'
            ...
            '5. 所有summary字段严格遵守上述结构要求和禁止中文方括号规则\n'
        )
        return prompt
    ...
    def _validate_scoring_result(self, scoring_json: dict) -> Tuple[bool, str]:
        ...
        # ── 字数校验（综合评估至少250字）──────────────────────────────────
        if len(summary["comprehensive_evaluation"]) < 250:
            return False, "综合评价字数不足250字"
    ...
    def _patch_summary_for_fallback(self, scoring_json: dict) -> dict:
        ...
        improvement_suggestions 为合法值，确保后续代码不崩溃。
        """
        score_info = self._collect_item_scores(scoring_json)
        has_full   = len(score_info["full_score_items"]) > 0
        all_full   = (score_info["total_items"] > 0 and
    ...
    def score_esg_report(self, esg_source: str = None, esg_text: str = None,
        ...
        # 修正 summary 中的综合评价（确保提及得分与评级，但保留原有详细内容）
        summary = scoring_json.setdefault("summary", {})
        ...
        # 若原文本未包含评级信息，则在开头补上
        if f"最终得分{correct_score}分" not in original_eval or f"评级为{correct_level}" not in original_eval:
            correct_prefix = f"最终得分{correct_score}分，评级为{correct_level}。"
            # 移除已有但不规范的开头评级描述
            cleaned_eval = re.sub(
                r'^(最终得分[\d.]+分[，,]?评级为[^\s，。]+[。，]?)',
                '',
            ...
            summary["comprehensive_evaluation"] = original_eval
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*60}")
    print(f"ESG碳信息披露评分系统 v6（新summary模板）")
    print(f"{'='*60}")
